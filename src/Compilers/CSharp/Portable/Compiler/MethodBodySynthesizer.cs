...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal static class MethodBodySynthesizer
    {
            ...
            CSharpCompilation compilation)
        {
            ...
            var submissionArrayReference = new(syntax, submissionConstructor.Parameters[0]) { WasCompilerGenerated = true };

            var intType = compilation.GetSpecialType(SpecialType.System_Int32);
            var objectType = compilation.GetSpecialType(SpecialType.System_Object);
            var thisReference = new(syntax, submissionConstructor.ContainingType) { WasCompilerGenerated = true };

            var slotIndex = compilation.GetSubmissionSlotIndex();
            Debug.Assert(slotIndex >= 0);

            // <submission_array>[<slot_index] = this;
            statements.Add(new(syntax,
                new(syntax,
                    new(syntax,
                        submissionArrayReference,
                        ImmutableArray.Create<BoundExpression>(new(syntax, ConstantValue.Create(slotIndex), intType) { WasCompilerGenerated = true }),
                        objectType)
                    { WasCompilerGenerated = true },
                    thisReference,
                    false,
            ...
            if ((object)hostObjectField != null)
            {
                ...
                statements.Add(
                    new(syntax,
                        new(syntax,
                            new(syntax, thisReference, hostObjectField, ConstantValue.NotAvailable) { WasCompilerGenerated = true },
                            BoundConversion.Synthesized(syntax,
                                new(syntax,
                                    submissionArrayReference,
                                    ImmutableArray.Create<BoundExpression>(new(syntax, ConstantValue.Create(0), intType) { WasCompilerGenerated = true }),
                                    objectType),
            ...
            foreach (var field in synthesizedFields.FieldSymbols)
            {
                ...
                statements.Add(
                    new(syntax,
                        new(syntax,
                            new(syntax, thisReference, field, ConstantValue.NotAvailable) { WasCompilerGenerated = true },
                            BoundConversion.Synthesized(syntax,
                                new(syntax,
                                    submissionArrayReference,
                                    ImmutableArray.Create<BoundExpression>(new(syntax, ConstantValue.Create(targetSubmissionIndex), intType) { WasCompilerGenerated = true }),
                                    objectType)
                                { WasCompilerGenerated = true },
                                Conversion.ExplicitReference,
                                false,
        ...
        internal static BoundBlock ConstructAutoPropertyAccessorBody(SourceMemberMethodSymbol accessor)
        {
            ...
            if (!accessor.IsStatic)
            {
                ...
                thisReference = new(syntax, thisSymbol.Type) { WasCompilerGenerated = true };
            }

            var field = property.BackingField;
            var fieldAccess = new(syntax, thisReference, field, ConstantValue.NotAvailable) { WasCompilerGenerated = true };
            BoundStatement statement;

            if (accessor.MethodKind == MethodKind.PropertyGet)
            {
                statement = new(accessor.SyntaxNode, RefKind.None, fieldAccess);
            }
            else
            {
                ...
                statement = new(
                    ...
                    new(
                        syntax,
                        fieldAccess,
                        new(syntax, parameter) { WasCompilerGenerated = true },
                        property.Type)
                    { WasCompilerGenerated = true });
            }
        ...
        internal static BoundBlock ConstructFieldLikeEventAccessorBody_WinRT(SourceEventSymbol eventSymbol, bool isAddMethod, CSharpCompilation compilation, DiagnosticBag diagnostics)
        {
            ...
            BoundFieldAccess fieldAccess = new(
                syntax,
                field.IsStatic ? null : new(syntax, accessor.ThisParameter.Type),
                field,
            ...
            else
            {
                ...
                //     return;
                // }  
                BoundStatement callStatement = new(syntax, processHandlerCall);
                BoundStatement returnStatement = new(syntax, RefKind.None, expressionOpt: null);
        ...
        internal static BoundBlock ConstructFieldLikeEventAccessorBody_Regular(SourceEventSymbol eventSymbol, bool isAddMethod, CSharpCompilation compilation, DiagnosticBag diagnostics)
        {
            ...
            BoundStatement @return = new(syntax,
            ...
            if (updateMethod == null)
            {
                ...
                diagnostics.Add(new(new(ErrorCode.ERR_MissingPredefinedMember,
            ...
            BoundThisReference fieldReceiver = eventSymbol.IsStatic ?
                null :
                new(syntax, thisParameter.Type) { WasCompilerGenerated = true };
            ...
            if ((object)compareExchangeMethod == null)
            {
                ...
                BoundStatement eventUpdate = new(syntax,
            ...
            {
                tmps[i] = new(accessor, TypeWithAnnotations.Create(delegateType), SynthesizedLocalKind.LoweringTemp);
                boundTmps[i] = new(syntax, tmps[i], null, delegateType) { WasCompilerGenerated = true };
            }

            // tmp0 = _event;
            BoundStatement tmp0Init = new(syntax,
                expression: new(syntax,
                    left: boundTmps[0],
            ...
            // LOOP:
            BoundStatement loopStart = new(syntax,
                label: loopLabel)
            { WasCompilerGenerated = true };

            // tmp1 = tmp0;
            BoundStatement tmp1Update = new(syntax,
                expression: new(syntax,
                    left: boundTmps[1],
                    right: boundTmps[0],
                    type: delegateType)
            ...
            BoundStatement tmp2Update = new(syntax,
                expression: new(syntax,
                    left: boundTmps[2],
                    right: delegateUpdate,
                    type: delegateType)
            ...
            // tmp0 = Interlocked.CompareExchange<DelegateType>(ref _event, tmp2, tmp1);
            BoundStatement tmp0Update = new(syntax,
                expression: new(syntax,
                    left: boundTmps[0],
            ...
            BoundExpression loopExitCondition = new(syntax,
            ...
            BoundStatement loopEnd = new(syntax,
        ...
        internal static BoundBlock ConstructDestructorBody(MethodSymbol method, BoundBlock block)
        {
            ...
            {
                BoundStatement baseFinalizeCall = new(
                    syntax,
