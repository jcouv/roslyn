            ...
            bool originalBodyNested)
        {
            ...
            else if (Analyze(compilation, method, block, diagnostics))
            {
                ...
                if (!hasTrailingExpression && ((object)submissionResultType != null))
                {
                    ...
                    var trailingExpression = new(method.GetNonNullSyntaxNode(), submissionResultType);
                    var newStatements = block.Statements.Add(new(trailingExpression.Syntax, RefKind.None, trailingExpression));
                    block = new(block.Syntax, ImmutableArray<LocalSymbol>.Empty, newStatements) { WasCompilerGenerated = true };
#if DEBUG
