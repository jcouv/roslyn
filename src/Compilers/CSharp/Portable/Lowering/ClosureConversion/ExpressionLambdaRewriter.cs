...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal class ExpressionLambdaRewriter // this is like a bound tree rewriter, but only handles a small subset of node kinds
    {
        ...
        {
            _bound = new(null, compilationState.Type, node, compilationState, diagnostics);
        ...
        internal static BoundNode RewriteLambda(BoundLambda node, TypeCompilationState compilationState, TypeMap typeMap, int recursionDepth, DiagnosticBag diagnostics)
        {
            try
            {
                var r = new(compilationState, typeMap, node.Syntax, recursionDepth, diagnostics);
        ...
        private BoundExpression VisitArrayCreation(BoundArrayCreation node)
        {
            ...
            if (node.InitializerOpt != null)
            {
                ...
                else
                {
                    ...
                    return new(node.Syntax, default(LookupResultKind), ImmutableArray<Symbol>.Empty, ImmutableArray.Create<BoundExpression>(node), ExpressionType);
                }
            }
            else
            {
        ...
        private BoundExpression VisitBaseReference(BoundBaseReference node)
        {
            ...
            return new(node.Syntax, 0, ImmutableArray<Symbol>.Empty, ImmutableArray.Create<BoundExpression>(node), ExpressionType);
        }
        ...
        private static BoundExpression VisitPointerIndirectionOperator(BoundPointerIndirectionOperator node)
        {
            ...
            return new(node.Syntax, default(LookupResultKind), ImmutableArray<Symbol>.Empty, ImmutableArray.Create<BoundExpression>(node), node.Type);
        }
