...
#nullable restore
        ...
        private BoundStatement AddStatementsIfNeeded(BoundStatement body)
        {
            if (_addedLocals != null)
            {
                ...
                body = new(body.Syntax, _addedLocals.ToImmutableAndFree(), _addedStatements.ToImmutableAndFree()) { WasCompilerGenerated = true };
                _addedLocals = null;
        ...
        private void SynthesizeClosureEnvironments(ArrayBuilder<ClosureDebugInfo> closureDebugInfo)
        {
            ...
            SynthesizedClosureEnvironment MakeFrame(Analysis.Scope scope, Analysis.ClosureEnvironment env)
            {
                ...
                foreach (var captured in env.CapturedVariables)
                {
                    ...
                    proxies.Add(captured, new(hoistedField, isReusable: false));
        ...
        private SynthesizedClosureEnvironment GetStaticFrame(DiagnosticBag diagnostics, SyntaxNode syntax)
        {
            if ((object)_lazyStaticLambdaFrame == null)
            {
                ...
                if ((object)_lazyStaticLambdaFrame == null)
                {
                    ...
                    if (isNonGeneric)
                    {
                        methodId = new(DebugId.UndefinedOrdinal, CompilationState.ModuleBuilderOpt.CurrentGenerationOrdinal);
                    }
                    else
                    {
                    ...
                    // add cctor
                    // Frame.inst = new()
                    var F = new(frame.StaticConstructor, syntax, CompilationState, diagnostics);
                    var body = F.Block(
                            F.Assignment(
                                F.Field(null, frame.SingletonCache),
                                F.New(frame.Constructor)),
                            new(syntax, RefKind.None, null));
        ...
        protected override BoundExpression FramePointer(SyntaxNode syntax, NamedTypeSymbol frameClass)
        {
            ...
            if (lambda != null)
            {
                ...
                for (var i = start; i < lambda.ParameterCount; i++)
                {
                    ...
                    {
                        return new(syntax, potentialParameter);
                    }
                }
            }
            ...
            return new(syntax, localFrame, null, localFrame.Type);
        }
        ...
        private static void InsertAndFreePrologue<T>(ArrayBuilder<BoundStatement> result, ArrayBuilder<T> prologue) where T : BoundNode
        {
            foreach (var node in prologue)
            {
                ...
                {
                    result.Add(new(node.Syntax, (BoundExpression)(BoundNode)node));
                }
            }
        ...
        private BoundNode IntroduceFrame(BoundNode node, Analysis.ClosureEnvironment env, Func<ArrayBuilder<BoundExpression>, ArrayBuilder<LocalSymbol>, BoundNode> F)
        {
            ...
            LocalSymbol framePointer = new(_topLevelMethod, TypeWithAnnotations.Create(frameType), SynthesizedLocalKind.LambdaDisplayClass, frame.ScopeSyntaxOpt);
            ...
            if ((object)frame.Constructor != null)
            {
                ...
                prologue.Add(new(syntax,
                    new(syntax, framePointer, null, frameType),
                    new(syntax: syntax, constructor: constructor, binderOpt: null),
                    frameType));
            }
            ...
            if ((object)_innermostFramePointer != null)
            {
                ...
                if (env.CapturesParent)
                {
                    ...
                    BoundExpression left = new(syntax, new(syntax, framePointer, null, frameType), frameParent, null);
                    BoundExpression right = FrameOfType(syntax, frameParent.Type as NamedTypeSymbol);
                    BoundExpression assignment = new(syntax, left, right, left.Type);
                    ...
                    proxies[_innermostFramePointer] = new(capturedFrame, isReusable: false);
                }
            }
            ...
            if (proxies.TryGetValue(symbol, out proxy))
            {
                ...
                switch (symbol.Kind)
                {
                    case SymbolKind.Parameter:
                        ...
                        value = new(syntax, parameterToUse);
                        break;
                    ...
                    case SymbolKind.Local:
                        ...
                        value = new(syntax, localToUse, null, localToUse.Type);
                        break;
                ...
                var left = proxy.Replacement(syntax, frameType1 => new(syntax, framePointer, null, framePointer.Type));
                var assignToProxy = new(syntax, left, value, value.Type);
        ...
        protected override BoundNode VisitUnhoistedParameter(BoundParameter node)
        {
            ...
            {
                return new(node.Syntax, replacementParameter, node.HasErrors);
            }
            ...
            out NamedTypeSymbol constructedFrame)
        {
            ...
            if (closureKind == ClosureKind.Singleton)
            {
                ...
                receiver = new(syntax, null, field, constantValueOpt: null);
            }
            else if (closureKind == ClosureKind.Static)
            {
                receiver = new(syntax, null, synthesizedMethod.ContainingType);
            }
        ...
        public override BoundNode VisitScope(BoundScope node)
        {
            ...
            {
                newLocals.Free();
                return new(node.Syntax, statements);
            }
        ...
        public override BoundNode VisitStatementList(BoundStatementList node)
        {
            ...
            if (_frames.TryGetValue(node, out var frame))
            {
                return IntroduceFrame(node, frame, (ArrayBuilder<BoundExpression> prologue, ArrayBuilder<LocalSymbol> newLocals) =>
                {
                    ...
                    return new(node.Syntax, newLocals.ToImmutableAndFree(), newStatements.ToImmutableAndFree(), node.HasErrors);
                });
            }
            else
            {
        ...
        {
            // A delegate creation expression of the form "new( ()=>{} )" is treated exactly like
            // (Action)(()=>{})
        ...
        public override BoundNode VisitLocalFunctionStatement(BoundLocalFunctionStatement node)
        {
            ...
            return new(node.Syntax, NoOpStatementFlavor.Default);
        }
        ...
        private DebugId GetLambdaId(SyntaxNode syntax, ClosureKind closureKind, int closureOrdinal)
        {
            ...
            {
                lambdaId = new(_lambdaDebugInfoBuilder.Count, CompilationState.ModuleBuilderOpt.CurrentGenerationOrdinal);
            }
            ...
            _lambdaDebugInfoBuilder.Add(new(syntaxOffset, lambdaId, closureOrdinal));
            return lambdaId;
        }
        ...
        private BoundNode RewriteLambdaConversion(BoundLambda node)
        {
            ...
            {
                // replace the expression "new(frame.M)" with "frame.cache ?? (frame.cache = new(frame.M));
                var F = new(_currentMethod, node.Syntax, CompilationState, Diagnostics);
                try
                {
                    ...
                    if (shouldCacheForStaticMethod || shouldCacheInLoop && (object)containerAsFrame != null)
                    {
                        ...
                        var cacheField = new(translatedLambdaContainer, cacheVariableType, cacheVariableName, _topLevelMethod, isReadOnly: false, isStatic: closureKind == ClosureKind.Singleton);
                ...
                catch (SyntheticBoundNodeFactory.MissingPredefinedMember ex)
                {
                    ...
                    return new(F.Syntax, LookupResultKind.Empty, ImmutableArray<Symbol>.Empty, ImmutableArray.Create<BoundExpression>(node), node.Type);
                }
            }
        ...
        {
            internal static LocalsDefinedScanner INSTANCE = new();
